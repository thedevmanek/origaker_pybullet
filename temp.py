...
def interactive_camera_placement(pos_scale=1.,
    ...
    dbg['upAxisIndex'] =  pb.addUserDebugParameter('toggle upAxisIndex', 0, 1, 1)
    ...
    target_vis_id = pb.createVisualShape(pb.GEOM_SPHERE,
                                         radius=0.01,
                                         rgbaColor=[1,0,0,0.7])
    target_body = pb.createMultiBody(baseMass=0, baseVisualShapeIndex=target_vis_id)

    camera_vis_id = pb.createVisualShape(pb.GEOM_BOX,
                                         halfExtents=[0.02, 0.05, 0.02],
                                         rgbaColor=[1,0,0,0.7])
    camera_body = pb.createMultiBody(baseMass=0, baseVisualShapeIndex=camera_vis_id)
    ...
    while 1:
        ...
        upAxisIndex = int(values['upAxisIndex'])
        ...
        if old_print_val != values['print']:
            ...
            if verbose:
                ...
                print(f"distance : {values['distance']:.2f}")
                print(f"yaw : {values['yaw']:.2f}")
                print(f"pitch : {values['pitch']:.2f}")
                print(f"roll : {values['roll']:.2f}")
                print(f"upAxisIndex : {upAxisIndex:d}")
                print(f"width : {width:d}")
                print(f"height : {height:d}")
                print(f"fov : {values['fov']:.1f}")
                print(f"aspect : {aspect:.2f}")
                print(f"nearVal : {values['near_val']:.2f}")
                print(f"farVal : {values['far_val']:.2f}")
            print("========================================\n")
...
def initialize_simulator():
    ...
    robot_id = pb.loadURDF("urdf/origaker.urdf",basePosition=[-2,0,0])
